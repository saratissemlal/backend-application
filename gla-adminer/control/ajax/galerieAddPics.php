...
if(!empty($_FILES)){
    ...
    if(!empty($_FILES['image']['name']) && !empty($_FILES['image']['tmp_name'])){
        ...
        $manager->make($_FILES['image']['tmp_name'])->orientate()->interlace(true)->save("../../../theme/assets/img/large/$name");

        $db->query("INSERT INTO image(nameI,typeI,sizeI,widthI,heightI) VALUES (?,?,?,?,?)",[$name,'site',$size,$width[0],$width[1]]);
